<template>
  <div>
    <el-select v-model="submit_data[data.key]" @change="onChange" @visible-change="onVisibleChange" @remove-tag="onRemoveTag" @clear="onClear" v-bind="select_attrs" :multiple='data.multiple ? true : false' :placeholder="data.desc">
      <el-option v-for='(item,index) in data.list' v-bind="option_attrs" :key='index' :value="item[value_attr]" :label="item[label_attr]"></el-option>
    </el-select>
  </div>
</template>
