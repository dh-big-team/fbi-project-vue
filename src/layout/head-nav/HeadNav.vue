      ...
      <el-row>
        ...
        <el-col :span="16">
          <el-menu :default-active="$store.state.router.headerCurRouter" mode="horizontal" background-color="#324157" text-color="#bfcbd9" active-text-color="#20a0ff" unique-opened router>
            ...
            <el-menu-item v-for='item in $router.options.routes' :index="item.path" :key='item.path' v-if='!item.hidden && (($store.state.user.userinfo.access_status===1 && $store.state.user.userinfo.web_routers[item.path]) || $store.state.user.userinfo.access_status!==1)'>
              {{item.name}}
              <!-- {{item.path}} -->
            </el-menu-item>
          </el-menu>
