<template>
  <div class='tabs'>
    <el-tabs v-model="$store.state.tabs.tabs_cur" type="card" closable @tab-click="onTabClick" @tab-remove="onRemoveTab">
      <el-tab-pane v-for="route in $store.state.tabs.list" :key="route.path" :label="route.name" :name="route.path">{{route.name}}
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
